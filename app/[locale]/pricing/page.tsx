        ...
        <Container>
          ...
          <StaggerChildren className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-12">
            {balanceSteps.map((step) => {
              ...
              return (
                <div
                  key={step.title}
                  className="bg-surface-1 border border-navy-100 rounded-2xl p-8 text-center hover:shadow-md hover:-translate-y-1 transition-all duration-300"
                >
          ...
          <ScrollReveal>
            <div className="mt-12 text-center">
              <Link
                href="/get-started"
                className="inline-flex items-center justify-center px-8 py-4 rounded-xl bg-primary-500 text-white font-semibold text-base hover:bg-primary-600 transition-colors shadow-sm"
              >
                Top Up & Start
              </Link>
            </div>
        ...
        <Container>
          <ScrollReveal>
            <div className="max-w-3xl mx-auto">
              <h2 className="text-navy-900 text-3xl lg:text-4xl font-bold tracking-tight text-center">
                Why balance-based pricing?
              </h2>
              <p className="text-navy-500 text-lg mt-4 text-center max-w-2xl mx-auto">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-10">
                {benefits.map((benefit) => (
                  <div key={benefit} className="flex items-center gap-3">
                    ...
                    <span className="text-navy-700 text-sm">{benefit}</span>
                  </div>
                ))}
              </div>
            </div>
