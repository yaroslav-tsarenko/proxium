...
import { posts, categoryColors } from "@/lib/blog";
          ...
          {posts.map((post) => {
            ...
            return (
              <Link
                key={post.slug}
                href={`/blog/${post.slug}`}
                className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden hover:border-zinc-700 hover:-translate-y-1 transition-all duration-300 group flex flex-col"
              >
                <div
                  className={`relative flex items-center justify-center h-36 bg-gradient-to-br ${post.gradient}`}
                >
                  <Icon className="w-10 h-10 text-zinc-50/80" strokeWidth={1.5} />
                </div>
